...
try
{
    ...
    builder.Services.ConfigureApplicationCookie(options =>
    {
        ...
        {
            if (context.Principal.IsInRole("Admin")) context.Response.Redirect("/Admin");
        };
    });
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        {
            var loggertwo = scope.ServiceProvider.GetRequiredService<ILogger<Program>>();
            loggertwo.LogError(ex, "An error occurred seeding the DB.");
        }
    }
...
{
    logger.Error(exception, "Aplikacja została zatrzymana z powodu nieoczekiwanego błędu");
    throw;
}
finally
{
